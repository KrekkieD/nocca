...
function readBody(readable, parse) {
    ...
    readable.on('end', function() {
        ...
        if (bufferArray.length) {
            ...
            //
            //if (readable.headers['content-encoding'] && readable.headers['content-encoding'] === 'gzip') {
            //    $zlib.gunzip(data, function (e, deflatedData) {
            //        deflatedData = deflatedData.toString();
            //        deflatedData = parse ? _parse(deflatedData) : deflatedData;
            //        deferred.resolve( deflatedData );
            //    });
            //}
            //else {
            //    data = parse ? _parse(data) : data;
            //}
        }
        else {
            deferred.resolve(data);
...
function unpack (reqContext) {



}

function pack (reqContext) {
