...
function WebsocketServer (Nocca) {
    ...
    function cleanConfig () {
        ...
        if (Nocca.config.servers.websocketServer.useHttpApiServer === true &&
			Nocca.config.servers.httpApi.enabled !== true) {
        ...
        if (Nocca.config.servers.websocketServer.useHttpApiServer === true) {
            Nocca.config.servers.websocketServer.options.httpServer = Nocca.httpApi.server;
        }
        else {
            // not reusing httpApi server, so create a new server
            var onTheFlyServer = $http.createServer();
            onTheFlyServer.listen(Nocca.config.servers.websocketServer.listen.port, function () {
                Nocca.logSuccess('Port:', Nocca.config.servers.websocketServer.listen.port, '-- WebsocketServer (on-the-fly style)');
            });

            // add the server to the websocket config
            Nocca.config.servers.websocketServer.options.httpServer = onTheFlyServer;
        }
