...
function Forwarder (Nocca) {
	...
	function prepareProxyRequest (reqContext) {
		...
		if (endpointDef) {
            ...
            // don't use url.resolve() to allow appending to the targetBaseUrl
			var resolvedUrl = endpointDef.targetBaseUrl + reqContext.endpoint.remainingUrl;
    ...
    function _performRequestForward (reqContext, deferred) {
        ...
        Nocca.logInfo('Forwarding request to: ' + proxyReq.host);
