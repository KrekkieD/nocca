    ...
    function log (reqContext) {
        ...
        if (typeof stats.responses[requestKeyHash] === 'undefined') {
            ...
            stats.responses[requestKeyHash] = {
                ...
                request: reqContext.getClientRequest().dump(),
                proxiedRequest: reqContext.getProxyRequest() ? reqContext.getProxyRequest().dump() : null,
                proxiedResponse: reqContext.getProxyResponse() ? reqContext.getProxyResponse().dump() : null,
                response: reqContext.getClientResponse() ? reqContext.getClientResponse().dump() : null,
                endpoint: reqContext.endpoint
            };

            // remove bodies for performance
            //delete stats.responses[requestKeyHash].request.bodies;
            //delete stats.responses[requestKeyHash].proxiedRequest.bodies;
            //delete stats.responses[requestKeyHash].proxiedResponse.bodies;
            //delete stats.responses[requestKeyHash].response.bodies;
