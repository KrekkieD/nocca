...
function Recorder (Nocca) {
    ...
    function simpleResponseRecorder (reqContext) {
        ...
        if (reqContext.config.recordingSubject !== false) {
            ...
            var acceptedRepositories = _.filter(reqContext.config.repositories, function (repository) {

                var recordingPlugin = repository;
                repository = Nocca.usePlugin(repository);

                var flagRecorded = repository.considerRecording(reqContext);

                if (flagRecorded) {
                    Nocca.logInfo('Proxied response was recorded into repository: ' + Nocca.pluginLoader.getPluginDefinition(recordingPlugin).name);
                }
            ...
            reqContext.flagRecorded = acceptedRepositories.length > 0;
