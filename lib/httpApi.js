...
function HttpApi (Nocca) {
    ...
    function getCaches (req, res, config) {
        _writeHead(res, 200).writeEnd(JSON.stringify(Nocca.playback.exportRecordings(), null, 4));
    }
    ...
    function addCachePackage (req, res, config) {
        $utils.readBody(req).then(function(body) {
            ...
            var recordings = Nocca.playback.exportRecordings();
    ...
    function getAllScenarios(req, res) {
        _writeHead(res).writeEnd(JSON.stringify(Nocca.playback.exportScenarios()));
    }
    ...
    function getScenarioByKey (req, res, config, params) {
        try {
            ...
            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.playback.exportScenarios(params.scenarioKey)));
    ...
    function getScenarioStatusByKey (req, res, config, params) {
        ...
        try {
            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.playback.exportScenarios(params.scenarioKey).$$active));
    ...
    function resetScenarioByKey(req, res, config, params) {
        try {
            Nocca.playback.resetScenario(params.scenarioKey);

            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.playback.exportScenarios(params.scenarioKey).currentPosition));
