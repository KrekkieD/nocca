...
function HttpApi (Nocca) {
    ...
    addRoute('GET:/scenarios/recorder', checkRecordingScenario);
    addRoute('PUT:/scenarios/recorder', changeRecorderState);
    addRoute('OPTIONS:/scenarios/recorder', allowRecorderCall);
    //addRoute('PUT:/scenarios/recordersave', stopRecordingScenario);
    ...
    function allowRecorderCall (req, res) {
        ...
        res.end();
        //res.write(JSON.stringify(isRecording), function () { res.end() });

    }

    function checkRecordingScenario (req, res) {

        var isRecording = Nocca.scenarioRecorder.isRecording();
        if (Nocca.scenarioRecorder.isRecording() !== false) {
            ...
            res.write('off', function () { res.end() });
        ...
        $utils.readBody(req, true)
            .then(function (body) {
                ...
                if (body.startRecording === true) {
                    ...
                    res.writeHead(400);
                    res.write('Bad body content', function () { res.end(); });
                }
    ...
    function startRecordingScenario (req, res, body) {
        try {
            ...
            res.write(JSON.stringify(builder.scenario), function() { res.end(); });
        } catch (e) {
            switch (e.id) {
                ...
                case $constants.ERRORS.INCOMPLETE_OBJECT:
                    res.writeHead(400, 'Supply a name and title for the new scenario');
                    break;
                default:
                    res.writeHead(500);
            }

            res.write(e.message, function() { res.end(); });
        }
    }
