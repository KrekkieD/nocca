...
function HttpApi (Nocca) {
    ...
    addRoute('POST:/scenarios?record', startRecordingScenario);
    addRoute('POST:/scenarios?save', stopRecordingScenario);
    ...
    function startRecordingScenario (req, res, config) {
        $utils.readBody(req, true).then(function(body) {
            ...
            res.setHeader('Location', '/scenarios/' + body.name);
    ...
    function stopRecordingScenario (req, res, config) {
        try {
            ...
            if (parsedUrl.query && parsedUrl.query.hasOwnProperty('save')) {
