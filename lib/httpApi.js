...
function HttpApi (Nocca) {
    ...
    function getCaches (req, res, config) {
        _writeHead(res, 200).writeEnd(JSON.stringify(Nocca.cacheentries.exportRecordings(), null, 4));
    }
    ...
    function addCachePackage (req, res, config) {
        $utils.readBody(req).then(function(body) {
            ...
            var recordings = Nocca.cacheentries.exportRecordings();
    ...
    function stopRecordingScenario (req, res, body) {
        try {
            ...
            // TODO: commented out, this should be added by the user and not automatically
            // Nocca.playback.addScenario(scenario.player());
    ...
    function getAllScenarios(req, res) {
        _writeHead(res).writeEnd(JSON.stringify(Nocca.cacheentries.exportScenarios()));
    }
    ...
    function getScenarioByKey (req, res, config, params) {
        try {
            ...
            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.cacheentries.exportScenarios(params.scenarioKey)));
    ...
    function getScenarioStatusByKey (req, res, config, params) {
        ...
        try {
            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.cacheentries.exportScenarios(params.scenarioKey).$$active));
    ...
    function resetScenarioByKey(req, res, config, params) {
        try {
            Nocca.cacheentries.resetScenario(params.scenarioKey);

            _writeHead(res)
                .writeEnd(JSON.stringify(Nocca.cacheentries.exportScenarios(params.scenarioKey).currentPosition));
