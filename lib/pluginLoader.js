...
function PluginLoader (Nocca) {
    ...
    function registerPlugins (configPlugins = []) {
        // initialise with default plugins
        var plugins = $globby.sync(__dirname + '/plugins/**/*.js');

        // add external plugins from configuration
        if (Array.isArray(configPlugins)) {
            plugins = plugins.concat(configPlugins);
        }

        plugins.forEach(function (plugin) {
            if (typeof plugin === 'string') {
                // load plugin from provided path, should return an object
    ...
    function instantiatePlugin (pluginId, pluginConfig) {
        ...
        // at this time it is always a single instance that is shared. Is this desirable?
