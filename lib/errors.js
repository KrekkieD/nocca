...
function defaultFailureHandlerFactory (reqContext) {
    ...
    httpResponse.writeHead(reqContext.statusCode || 500, {
        'Nocca-Error': '"' + (reqContext.error || reqContext.message) + '"'
    });

    console.log('|    Unable to complete request: ' + (reqContext.error || reqContext.message));
    if (reqContext.stack) {
        console.error(reqContext.stack);
    }

    httpResponse.end(reqContext.error || reqContext.message);
