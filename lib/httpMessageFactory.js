...
function HttpMessageFactory (Nocca) {
    ...
    parentSelf.logger = Nocca.logger.child({ module: 'HttpMessageFactory' });
    ...
    function readIncomingMessage (req) {
        ...
        parentSelf.logger.debug('reading incoming body');
        return $utils.readBody(req)
            .then(function (bodyBuffer) {
                self.setBody(bodyBuffer, 'raw');
                parentSelf.logger.debug('incoming body read');
            ...
            .then(function () {
                ...
                if (self.headers.hasOwnProperty('content-length')) {
                    ...
                    if (bodyLength !== contentLength) {
                        parentSelf.logger.warn('Content-Length header mismatches actual body size, adjusting header');
