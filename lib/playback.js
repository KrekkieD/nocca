...
function combineMatchers() {
    ...
    var matcherFunction = function (reqContext) {
        ...
        for (var i = 0; i < matchers.length && !reqContext.playbackResponse; i++) {
...
function simpleRequestKeyRequestMatcher (reqContext) {
    ...
    if (typeof recordings[reqContext.endpoint.key] !== 'undefined' &&
        ...
        //Nocca.logInfo('Found a matching record!');

    }
    else {
        // Probably no recording found, just return the context as is
        //Nocca.logDebug('No matching record found');
    }
...
function scenarioBasedRequestMatcher(reqContext) {
    ...
    if (potentialScenarios.length > 0) {
        ...
        var playbackResponse = Nocca.httpMessageFactory.createResponse(potentialScenarios[0].currentPosition.state.response);

        reqContext.setPlaybackResponse(playbackResponse);

        Nocca.logDebug('Playing scenario request: ' + potentialScenarios[0].scenario.title + ' -- ' + potentialScenarios[0].currentPosition.state.title);
