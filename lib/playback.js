    ...
    function findMatchingRequest(reqContext) {
        ...
        for (var i = 0; !reqContext.getPlaybackResponse() && i < Nocca.repositories.length; i++) {
            ...
            if (repositoryPlaybackEnabled[Nocca.repositories[i].name()]) {
                Nocca.logDebug('checking caches in repository: ' + Nocca.repositories[i].name());
                var playbackResponse = Nocca.repositories[i].matchRequest(reqContext);
                if (playbackResponse) {
                    reqContext.setPlaybackResponse(playbackResponse);
                    Nocca.logDebug('Cache found in repository: ' + Nocca.repositories[i].name());
                }
            }
