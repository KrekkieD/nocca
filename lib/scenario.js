...
function Scenario(name, title) {
    ...
    this.type = $nocca.$constants.SCENARIO_TYPE_SEQUENTIAL;
    this.repeatable = $nocca.$constants.SCENARIO_REPEATABLE_INFINITE;
}
...
function Serializer(scenario) {
    ...
    if (scenario.type === $nocca.$constants.SCENARIO_TYPE_SEQUENTIAL) {
        builderCode += "    .sequentialScenario()\n";
    }
    
    if (scenario.repeatable === $nocca.$constants.SCENARIO_REPEATABLE_ONE_SHOT) {
        builderCode += "    .oneShot()\n";
    }
    else if (scenario.repeatable === $nocca.$constants.SCENARIO_REPEATABLE_INFINITE) {
...
// -- Scenario Type
Builder.prototype.sequentialScenario = builderSetter('scenario', 'type', false, $nocca.$constants.SCENARIO_TYPE_SEQUENTIAL);

// -- Scenario Repeatability
Builder.prototype.oneShot = builderSetter('scenario', 'repeatable', false, $nocca.$constants.SCENARIO_REPEATABLE_ONE_SHOT);
Builder.prototype.infiniteLoop = builderSetter('scenario', 'repeatable', false, $nocca.$constants.SCENARIO_REPEATABLE_INFINITE);
...
ScenarioPlayer.prototype.next = function() {
    ...
    if (eligibleEdges.length === 0) {
        ...
        if (this.scenario.repeatable === $nocca.$constants.SCENARIO_REPEATABLE_ONE_SHOT) {
            this.finished = true;
        }
        else { // if (this.repeatable === $nocca.$constants.SCENARIO_REPEATABLE_INFINITE) {
            this.reset();
        }
    }
    else {
