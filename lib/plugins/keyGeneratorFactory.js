...
function KeyGeneratorFactory (Nocca) {
    ...
    function generateKey (reqContext) {
        ...
        if (typeof reqContext.config.keyGeneratorFactoryMethod !== 'undefined') {
            keyGenToUse = reqContext.config.keyGeneratorFactoryMethod;
        }

        return self[keyGenToUse].apply(null)(reqContext);
