...
// This repository explicitly depends on the internal scenario and scenarioRecorder (other repositories may depend on other recorders)
var $scenario = require('./scenarioRepository/scenario');
var $scenarioRecorder = require('./scenarioRepository/scenarioRecorder');
...
module.exports = {
    ...
    description: 'Fixed-order, no-slack scenario recorder and player',
...
function ScenarioRepository (Nocca) {
    ...
    var scenario = new $scenario(Nocca);
...
// --  Import Data
    ...
    function importInitialScenarios () {
        ...
        for (var i = 0, iMax = Nocca.config.scenarios.available.length; i < iMax; i++) {
            ...
            var recordedScenario = Nocca.config.scenarios.available[i](scenario);
            addSingleScenario(recordedScenario.player());
...
// --  Scenario based playback (includes adding scenarios to the datastore)
    ...
    function matchRequest(reqContext) {
        ...
        if (potentialScenarios.length > 0) {
            ...
            self.logger.info('Playing scenario request: ' + potentialScenarios[0].scenario.title + ' -- ' + potentialScenarios[0].currentPosition.state.title);
            ...
            self.logger.debug('No matching record found in scenarios');
        }
    ...
    function addSingleScenario(scenarioPlayer) {
        ...
        if (typeof scenarios[scenarioPlayer.scenario.name] !== 'undefined') {
            throw 'A scenario with name "' + scenarioPlayer.scenario.name + '" was already loaded!';
        }

        self.logger.info('Saving new scenario to player: ' + scenarioPlayer.scenario.title + ' (' + scenarioPlayer.scenario.name + ')');
