...
function CacheEntries (Nocca) {
    ...
    self.logger = Nocca.logger.child({ module: 'CacheEntries' });
...
// --- Simple recording playback
    ...
    function considerRecordingRequest(reqContext) {
        ...
        addSingleRecording(
            ...
            reqContext.getHttpMessage(reqContext.config.recordingSubject).dump()
        );
    ...
    function hashRequestKey(requestKey) {
    ...
    function matchRequest (reqContext) {
        ...
        if (typeof recordings[reqContext.endpoint.key] !== 'undefined' &&
            ...
            self.logger.info('Found a matching record using simpleMatcher!');

        }
        else {
            // Probably no recording found, just return the context as is
            self.logger.info('No matching record found using simpleMatcher');
        }
...
// --- REST control methods
    ...
    function initRestRoutes() {
        ...
        var routes = [
            ['GET:/repositories/memory-caches/endpoints', getCachesForAllEndpoints],
            ['DELETE:/repositories/memory-caches/endpoints', clearAllCaches],

            ['GET:/repositories/memory-caches/endpoints/:endpointKey/caches', true, getCacheEntriesForEndpoint],
            ['DELETE:/repositories/memory-caches/endpoints/:endpointKey/caches', true, deleteCacheEntriesForEndpoint],
            ['POST:/repositories/memory-caches/endpoints/:endpointKey/caches', true, saveOrReplaceCacheEntry],

            ['GET:/repositories/memory-caches/endpoints/:endpointKey/caches/:requestKeyHash', true, getCacheEntryByKey],
            ['PUT:/repositories/memory-caches/endpoints/:endpointKey/caches/:requestKeyHash', true, saveOrReplaceCacheEntry],
            ['DELETE:/repositories/memory-caches/endpoints/:endpointKey/caches/:requestKeyHash', true, deleteCacheEntry],

            ['POST:/caches/package', addCachePackage]
        ];

        // publish event for each routes
    ...
    function getCachesForAllEndpoints (apiReq) {
        apiReq.ok().end(recordings);
    }

    function clearAllCaches (apiReq) {

        // clear all keys in the recordings object
        Object.keys(recordings).forEach(function (key) {
            delete recordings[key];
        });

        // then respond with the usual GET
        getCachesForAllEndpoints(apiReq);
    ...
    function getCacheEntriesForEndpoint (apiReq) {
        ...
        if (typeof recordings[apiReq.matches.endpointKey] !== 'undefined') {
            ...
            apiReq.ok().end(recordings[apiReq.matches.endpointKey]);
        }
        else {
            apiReq.notFound().end();
        }
    ...
    function deleteCacheEntriesForEndpoint (apiReq) {

        if (typeof recordings[apiReq.matches.endpointKey] !== 'undefined') {

            Object.keys(recordings[apiReq.matches.endpointKey]).forEach(function (requestKeyHash) {
                delete recordings[apiReq.matches.endpointKey][requestKeyHash];
            });
    ...
    function getCacheEntryByKey (apiReq) {
        ...
        if (typeof recordings[apiReq.matches.endpointKey] !== 'undefined' &&
            ...
            apiReq.ok().end(recordings[apiReq.matches.endpointKey][apiReq.matches.requestKeyHash]);
        }
        else {
            apiReq.notFound().end();
        }
